...
for ($i = 0; $i < 500; $i++) {
    ...
    foreach ($list as $key => $token) {
        ...
        if (strpos($cekuser, '"ok"') !== false) {
            ...
            $bet_ganjil = curl('https://api2.idgame365.com/api/order/create_oe','{"type":2,"game_type":24,"money":"1001"}',$token);
            // print_r($bet_ganjil);
            ...
            $bet_genap = curl('https://api2.idgame365.com/api/order/create_oe','{"type":2,"game_type":24,"money":"1000"}',$token);
            // print_r($bet_genap);
