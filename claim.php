...
echo color('green',"[" . date("H:i:s") . "] ")."Input Kode Claim : ";
$codeclaim = trim(fgets(STDIN));
...
for ($i=0; $i <500 ; $i++) { 
    ...
    if(strpos($cekuser,'"ok"')){
        ...
        // $codeclaim = transliterator_get_error_message() ;
        

        $claimcode = curl('https://api2.idgame365.com/api/activity/active_code', '{"verification_code":"' . $codeclaim . '"}', $token);
        echo $claimcode;
